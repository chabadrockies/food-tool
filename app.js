...
function menuTile(index, quantity) {
  ...
  return `<article class="menu-tile"><button class="tile-add" type="button" onclick="addItem(${index})" aria-label="Add ${safeName}"><div class="item-name">${safeName}</div>${quantity ? '' : '<div class="add-hint">Tap to add</div>'}</button>${quantity ? `<div class="quantity-bar"><span class="selected-label">Added</span><div class="quantity-control"><button type="button" aria-label="Remove one ${safeName}" onclick="changeQuantity(${index},-1)">-</button><span class="quantity">${quantity}</span><button type="button" aria-label="Add one ${safeName}" onclick="changeQuantity(${index},1)">+</button></div></div>` : ''}</article>`;
}
...
function render() {
  ...
  if (!order) {
    ...
    document.getElementById('special-orders').hidden = true;
...
function addSpecialItem(name, price) {
  ...
  items.push(item);
  Object.values(orders).forEach(order => order.quantities.push(0));
  activeOrder().quantities[items.length - 1] = 1;
  document.getElementById('special-order-form').reset();
  status.textContent = `Created ${cleanName} and added one to ${dateLabel(activeDate)}.`;
  render();
}
...
function syncStickyLayout() {
  ...
  const height = document.querySelector('.date-manager').getBoundingClientRect().height;
  document.documentElement.style.setProperty('--date-manager-height', `${Math.ceil(height)}px`);
}
